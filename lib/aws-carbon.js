...
export function calculateInstanceCarbon(instance, cpuBuckets, profile, intensitySeries) {
  if (!profile || !cpuBuckets.length) {
    return { kilograms: null, kwh: null, buckets: 0, usageBuckets: [] };
  }
  ...
  for (const bucket of cpuBuckets) {
    ...
    usageBuckets.push({
      ...
      cpuUtilisation: bucket.cpuUtilisation,
      watts,
    });
...
export async function getAwsInventory({ roleArn, externalId, profiles }) {
  ...
  const endTime = new Date(Math.floor(Date.now() / HOUR_MS) * HOUR_MS);
  const startTime = new Date(endTime.getTime() - LOOKBACK_HOURS * HOUR_MS);
  ...
  return {
    ...
    warnings: [
      ...
      ...(rows.some((row) => row.buckets === 0) ? ["No completed hourly CPU data was available for one or more instances."] : []),
    ],
