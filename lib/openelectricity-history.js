...
const SNAPSHOT_FILE_NAME = "openelectricity-history.json";
const SNAPSHOT_FILE_PATH = path.join(process.cwd(), SNAPSHOT_FILE_NAME);
...
async function readSnapshotFile() {
  try {
    const snapshot = await fs.readFile(SNAPSHOT_FILE_PATH, "utf8");
...
async function writeSnapshotFile(snapshot) {
  const tempFilePath = `${SNAPSHOT_FILE_PATH}.tmp`;
  const content = `${JSON.stringify(snapshot, null, 2)}\n`;

  await fs.writeFile(tempFilePath, content, "utf8");
  await fs.rename(tempFilePath, SNAPSHOT_FILE_PATH);
}
...
async function refreshSnapshot() {
  ...
  if (!apiKey) {
    throw new Error(
      "Missing Open Electricity API key. Set OPEN_ELECTRICITY_API_KEY in .env.",
    );
  }
...
export async function getHistoricalGenerationSnapshot(options = {}) {
  ...
  const config = getRequestConfig();
  const cachedSnapshot = await readSnapshotFile();
  const stale = isSnapshotStale(cachedSnapshot, config.cacheMaxAgeHours);

  if (!forceRefresh && cachedSnapshot && !stale) {
    return {
